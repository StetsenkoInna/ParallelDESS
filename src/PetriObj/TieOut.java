 ...
 */
public class TieOut implements Cloneable, Serializable {
     ...
     */
    public TieOut(int T, int P, int K) {
        numP = P;
        numT = T;
        k = K;
        number = next;
     ...
     * @param K arc multiplicity
     */
    public TieOut(PetriT T, PetriP P, int K) {
        numP = P.getNumber();
        numT = T.getNumber();
        k = K;
        nameP = P.getName();
     ...
     */
    @Override
    public TieOut clone() throws CloneNotSupportedException {
        super.clone();
        TieOut tie = new TieOut(numT, numP, k); // коректніть номерів дуже важлива!!!
        return tie;
