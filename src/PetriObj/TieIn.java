 ...
 */
public class TieIn implements Cloneable, Serializable {
     ...
     *
     */
    public TieIn(int P, int T, int K) {
        numP = P;
        numT = T;
        k = K;
        inf = false;
     ...
     * @param T number of transition
     */
    public TieIn(PetriP P, PetriT T) {
        numP = P.getNumber();
        numT = T.getNumber();
        k = 1;
        inf = false;
     ...
     * @param K arc multiplicity
     */
    public TieIn(PetriP P, PetriT T, int K) {
        numP = P.getNumber();
        numT = T.getNumber();
        k = K;
        inf = false;
     ...
     */
    public TieIn(PetriP P, PetriT T, int K, boolean isInf) {
        numP = P.getNumber();
        numT = T.getNumber();
        k = K;
        inf = isInf;
     ...
     */
    @Override
    public TieIn clone() throws CloneNotSupportedException {
        super.clone();
        TieIn tie = new TieIn(numP, numT, k);  // коректніть номерів дуже важлива!!!
        return tie;
